...
class NeMoGymAsyncOpenAI(BaseModel):  # pragma: no cover
    ...
    async def create_response(self, **kwargs):
        request_kwargs = dict(
            ...
            json=kwargs,
            # Per-call tight timeout on the upstream focal LLM call: a stalled read
            # breaks in ~120s so the retry loop self-heals. The global client stays
            # permissive (server_utils.py) so the long /run + tool calls never
            # spuriously time out and retry (which would re-launch the marketplace).
            timeout=ClientTimeout(total=300, sock_read=120),
        )
        response = await self._request(method="POST", **request_kwargs)
