...
class ChannelEvent:
    ...
    Phase 1/2 fields: turn, event_id, agent, action, target, price, message.
    Phase 3 adds: wants (categories the lister accepts in trade),
                  image_path (photo of the listed item),
                  swap_item_id (proposer's item in a swap_proposal).
    """
    turn: int
    ...
    action: str              # "listing" | "offer" | "counter" | "accept" | "decline" | "pass" |
                             #   phase 3: "swap_proposal" | "accept_swap" | "reject_swap"
    target: Optional[str]    # referenced listing_id, offer_id, or item_id
    price: Optional[float]
    message: str
    timestamp: str = field(default_factory=lambda: datetime.utcnow().isoformat())
    # Phase 3 optional fields — None for phase 1/2 events
    wants: Optional[list] = None         # list of category strings for listings
    image_path: Optional[str] = None     # photo of the listed item
    swap_item_id: Optional[str] = None   # proposer's item_id in swap_proposal
