#!/usr/bin/env bash
# Run Phase 3 end-to-end: pre-flight, rollouts, archive, aggregate, snapshot.
#
# Usage:
#   bash scripts/run_phase_3.sh validate # 2 tasks (smoke test during development)
#   bash scripts/run_phase_3.sh 5task    # 5 tasks (1 per set, focal_S_vs_H, seed 42)
#   bash scripts/run_phase_3.sh 20task   # 20 tasks (5 sets × 4 configs × 1 focal × seed 42)
#
# Assumptions:
...
#   2. Set MARKETPLACE_PHASE=3 → personas_phase3/, agent_template_phase{3}*.txt
...
#   6. Snapshot the whole thing into results/phase3/run_*_TIMESTAMP/
...
export MARKETPLACE_PHASE=3

# --- pre-flight ---------------------------------------------------------

echo "============================================================"
echo "  Phase 3 — mode: $MODE   (MARKETPLACE_PHASE=$MARKETPLACE_PHASE)"
echo "============================================================"
...
# Resolve task file
if [ "$MODE" = "validate" ]; then
  TASKS_FILE="tasks/phase3_validate.jsonl"
  EXPECTED_LINES=2
  SNAPSHOT_KIND="validate"
  SNAPSHOT_CONFIG_TAG="focalSvH"
  SNAPSHOT_SEED_TAG="seed42"
elif [ "$MODE" = "5task" ]; then
  TASKS_FILE="tasks/phase3_5task.jsonl"
  EXPECTED_LINES=5
  SNAPSHOT_KIND="5tasks"
  SNAPSHOT_CONFIG_TAG="focalSvH"
  SNAPSHOT_SEED_TAG="seed42"
elif [ "$MODE" = "20task" ]; then
  TASKS_FILE="tasks/phase3_20task.jsonl"
  EXPECTED_LINES=20
...
if [ ! -f "$TASKS_FILE" ]; then
  ...
  if [ "$MODE" = "validate" ]; then
    echo "         MARKETPLACE_PHASE=3 .venv/bin/python tasks/generate_tasks.py \\"
    echo "             --phase 3 --focal-count 1 --config-filter focal_S_vs_H --seeds 42 \\"
    echo "             --out tasks/phase3_validate.jsonl"
    echo "         # then trim to 2 tasks for smoke (or write a 2-task subset directly)"
  elif [ "$MODE" = "5task" ]; then
    echo "         MARKETPLACE_PHASE=3 .venv/bin/python tasks/generate_tasks.py \\"
    echo "             --phase 3 --focal-count 1 --config-filter focal_S_vs_H --seeds 42 \\"
    echo "             --out tasks/phase3_5task.jsonl"
  else
    echo "         MARKETPLACE_PHASE=3 .venv/bin/python tasks/generate_tasks.py \\"
    echo "             --phase 3 --focal-count 1 --seeds 42 \\"
    echo "             --out tasks/phase3_20task.jsonl"
  fi
  exit 1
fi
...
mkdir -p results/phase_3 results/runs results/aggregates tasks/batches "results/phase1"

# --- split task file by focal model -------------------------------------

SONNET_BATCH="tasks/batches/phase3_${MODE}_sonnet_focal.jsonl"
HAIKU_BATCH="tasks/batches/phase3_${MODE}_haiku_focal.jsonl"
...
SONNET_OUT="results/phase_3/phase3_${MODE}_sonnet_focal_rollouts.jsonl"
HAIKU_OUT="results/phase_3/phase3_${MODE}_haiku_focal_rollouts.jsonl"
...
if [ "$N_SONNET" -gt 0 ]; then
  ...
  .venv/bin/ng_collect_rollouts \
    ...
    +limit="$N_SONNET" \
    +num_repeats=1 \
    +resume_from_cache=True 2>&1 | tee -a output_phase3.log
fi
...
echo "============================================================"
.venv/bin/python analysis/compare.py --phase 3
echo "✓ aggregator wrote results/aggregates/phase_3_summary.json"
...
# --- Snapshot into results/phase3/run_*_TIMESTAMP/ --------------------

echo ""
echo "============================================================"
echo "  Snapshotting run into results/phase3/"
echo "============================================================"
if [ -s "$SONNET_OUT" ]; then
...
.venv/bin/python -c "
import json
data = json.load(open('results/aggregates/phase_3_summary.json'))
print()
