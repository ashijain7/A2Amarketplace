#!/usr/bin/env bash
# Run Phase 1 end-to-end: pre-flight, rollouts, archive, aggregate, snapshot.
#
# Usage:
#   bash scripts/run_phase_1.sh 5task    # 5 tasks (1 per set, focal_S_vs_H, seed 42)
#   bash scripts/run_phase_1.sh 20task   # 20 tasks (5 sets × 4 configs × 1 focal × seed 42)
#
# Assumptions:
...
#   2. Set MARKETPLACE_PHASE=1 → personas_phase1/, agent_template_phase{1}*.txt
...
#   6. Snapshot the whole thing into results/phase1/run_*_TIMESTAMP/
...
echo "============================================================"
echo "  Phase 1 — mode: $MODE   (MARKETPLACE_PHASE=$MARKETPLACE_PHASE)"
echo "============================================================"
...
if [ "$MODE" = "5task" ]; then
  ...
  echo "ERROR: unknown mode '$MODE'. Use '5task' or '20task'."
  exit 1
fi
...
if [ ! -f "$TASKS_FILE" ]; then
  ...
  if [ "$MODE" = "5task" ]; then
    echo "         MARKETPLACE_PHASE=1 .venv/bin/python tasks/generate_tasks.py \\"
    echo "             --phase 1 --focal-count 1 --config-filter focal_S_vs_H --seeds 42 \\"
    echo "             --out tasks/phase1_5task.jsonl"
  else
    echo "         MARKETPLACE_PHASE=1 .venv/bin/python tasks/generate_tasks.py \\"
    echo "             --phase 1 --focal-count 1 --seeds 42 \\"
    echo "             --out tasks/phase1_20task.jsonl"
  fi
  exit 1
fi
...
mkdir -p results/phase_1 results/runs results/aggregates tasks/batches "results/phase1"

# --- split task file by focal model -------------------------------------

SONNET_BATCH="tasks/batches/phase1_${MODE}_sonnet_focal.jsonl"
HAIKU_BATCH="tasks/batches/phase1_${MODE}_haiku_focal.jsonl"
...
SONNET_OUT="results/phase_1/phase1_${MODE}_sonnet_focal_rollouts.jsonl"
HAIKU_OUT="results/phase_1/phase1_${MODE}_haiku_focal_rollouts.jsonl"
...
if [ "$N_SONNET" -gt 0 ]; then
  ...
  .venv/bin/ng_collect_rollouts \
    ...
    +limit="$N_SONNET" \
    +num_repeats=1 \
    +resume_from_cache=True 2>&1 | tee -a output_phase1.log
fi
...
echo "============================================================"
.venv/bin/python analysis/compare.py --phase 1
echo "✓ aggregator wrote results/aggregates/phase_1_summary.json"
...
# --- Snapshot into results/phase1/run_*_TIMESTAMP/ --------------------

echo ""
echo "============================================================"
echo "  Snapshotting run into results/phase1/"
echo "============================================================"
if [ -s "$SONNET_OUT" ]; then
